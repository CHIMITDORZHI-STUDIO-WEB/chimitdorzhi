/* Единый поиск по сайту (статьи + услуги + предложения).
   Самодостаточный виджет: сам вставляет модалку и стили, если их нет на странице.
   На главной модалка уже встроена — там этот файл не подключаем. */
(function () {
  if (window.__searchWidgetInit) return;
  window.__searchWidgetInit = true;

  function ensureStyle() {
    if (document.getElementById('searchWidgetStyle')) return;
    var css = '.search-modal{position:fixed;inset:0;z-index:10010;}'
  ...
  function ensureModal() {
    ...
    wrap.innerHTML =
      ...
      + '<input id="navSearchInput" type="search" placeholder="Поиск по статьям, услугам и предложениям…" autocomplete="off" aria-label="Поиск по сайту">'
  ...
  function init() {
    ...
    function load(cb) {
      ...
      loading = true;
      fetch('/search-index.json').then(function (r) { return r.json(); }).then(function (d) { idx = d; cb(); if (pending) { var p = pending; pending = null; p(); } }).catch(function () { });
    }
    function render(q) {
      var t = q.trim().toLowerCase();
      if (t.length < 2) { box.innerHTML = '<div class="search-empty">Введите минимум 2 символа</div>'; return; }
      var w = t.split(/\s+/);
      var res = idx.filter(function (it) { var hay = (it.t + ' ' + it.d + ' ' + (it.g || '') + ' ' + (it.c || '')).toLowerCase(); return w.every(function (x) { return hay.indexOf(x) >= 0; }); });
      var n = res.length;
      // релевантность: совпадение в заголовке важнее тела; услуги/предложения слегка поднимаем, чтобы не тонули в статьях
      function score(it) {
        var ht = it.t.toLowerCase(), hg = (it.g || '').toLowerCase(), hd = (it.d || '').toLowerCase(), s = 0;
        w.forEach(function (x) { if (ht.indexOf(x) >= 0) s += 10; if (hg.indexOf(x) >= 0) s += 3; if (hd.indexOf(x) >= 0) s += 1; });
        if (it.k === 'Услуга') s += 4; else if (it.k === 'Предложение') s += 2;
        return s;
      }
      res.sort(function (a, b) { return score(b) - score(a); });
      res = res.slice(0, 20);
      if (!n) { box.innerHTML = '<div class="search-empty">Ничего не найдено</div>'; return; }
      box.innerHTML = res.map(function (it) { return '<a class="search-item" href="' + it.u + '"><span class="search-kind">' + esc(it.k) + (it.c ? ' · ' + esc(it.c) : '') + '</span><span class="search-t">' + esc(it.t) + '</span><span class="search-d">' + esc(it.d).slice(0, 110) + '</span></a>'; }).join('') + (n > 20 ? '<div class="search-empty">+ ещё ' + (n - 20) + ' — уточните запрос</div>' : '');
    }
    ...
    var deb;
    inp.addEventListener('input', function () { clearTimeout(deb); var q = inp.value; deb = setTimeout(function () { load(function () { render(q); }); }, 110); });
  }
