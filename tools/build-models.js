...
function head({ title, description, url, ld }) {
  ...
  h += '<link rel="stylesheet" href="/assets/models.css?v=1">\n';
...
function detail(m) {
  ...
  <div class="md-d-body">
    <div class="md-d-main">
      ...
      ${alts.length ? `<section><h2>Похожие модели</h2><div class="md-alts">${alts.map((a) => `<a href="/ii-modeli/${a.id}/"><b>${esc(a.name)}</b><span>${esc(a.developer)} · ${lic(a.commercial)}</span></a>`).join('')}</div></section>` : ''}
    </div>
