...
module.exports = [
  ...
  // иначе получится дубль slug.
  {
    ...
    metaDescription: 'Голосовой робот обзванивает базу и принимает входящие вместо оператора: напоминания, подтверждение заказов, опросы. Когда окупается, что нужно для запуска и сколько стоит.',
  ...
  },
  {
    ...
    metaDescription: 'Как настроить автоматический мониторинг закупок по 44-ФЗ: фильтры по ОКПД и региону, уведомления в мессенджер за минуты. Готовые сервисы или свой парсер — что выбрать.',
  ...
  },
  {
    ...
    metaDescription: 'Как связать свою учётную систему с кабинетами WB и Ozon через API: автообновление остатков и цен, сбор отзывов, контроль штрафов. Что даёт API и где его пределы.',
