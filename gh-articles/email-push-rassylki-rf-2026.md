...
**Я перенёс базу из Mailchimp — можно сразу запускать полноценную рассылку?** Перенести можно, но сразу слать большой объём нельзя — нужен прогрев нового домена/сервиса. И проверьте, что на всех контактов есть согласие по ФЗ-38, иначе рискуете жалобами и штрафом.

**Какой российский сервис рассылок выбрать, если я только начинаю?** Unisender или DashaMail для массовых рассылок. Для транзакционных писем из приложения — Rusender или Notisend. Web-push поднимайте сами на VAPID.

**Нужно ли получать согласие пользователя на транзакционные письма вроде кода подтверждения?** Нет. Подтверждение заказа, код, чек, восстановление пароля — это не реклама, пользователь сам инициировал действие. Согласие нужно только на рекламные и триггерно-рекламные письма.

**Мне лучше поднять свой SMTP-сервер или взять готовый сервис рассылок?** Для большинства — готовый сервис. Свой SMTP оправдан от больших объёмов транзакционки или ради полной независимости, и требует постоянного внимания к репутации.

**Нужно ли маркировать рекламные рассылки через ОРД и указывать erid?** Если рассылаете своё своим подписчикам — как правило, нет (но согласие и отписка обязательны). Если размещаете рекламу третьих лиц — да, через ОРД. При сомнениях — к юристу.

**Почему мои письма стабильно попадают в спам?** Чаще всего: не настроены или сломаны SPF/DKIM/DMARC, нет прогрева, грязная база, много жалоб, спрятана отписка. Проверьте постмастеры и mail-tester по очереди.

**Безопасно ли в 2026 году использовать Sendpulse для рассылок из России?** Технически работает, но у него украинское происхождение — есть вопросы по юрисдикции, оплате и 152-ФЗ. При прочих равных я выбираю российский аналог.

**Можно ли отправлять push-уведомления пользователю без его согласия?** Web-push требует явного разрешения в браузере — пользователь сам нажимает «разрешить», это и есть согласие. Mobile push — аналогично, пользователь разрешает уведомления в приложении. Но рекламный характер push-сообщений всё равно подпадает под общую логику ФЗ-38.

**Firebase точно продолжает работать для push-уведомлений в России?** На большинстве устройств да, но это зарубежная зависимость с риском. Для новых российских приложений закладывайте RuStore Push и резервный канал.
