...
**Коротко:** большая языковая модель не понимает текст как человек — она предсказывает наиболее вероятный следующий токен, опираясь на статистические закономерности языка, выученные при обучении на огромных массивах текста. Это предсказание, доведённое до огромного масштаба, порождает поведение, похожее на понимание: связные ответы, объяснения, переводы. Но у модели нет проверки истинности, поэтому она может уверенно галлюцинировать, а в критичных для бизнеса сценариях ответы нужно проверять человеком.
...
**Почему у модели ограниченный размер контекста и что это значит?** Модель обрабатывает не бесконечный текст, а ограниченное окно из определённого числа токенов, которые она «видит» одновременно при генерации ответа. Всё, что осталось за пределами этого окна, для модели в момент ответа попросту не существует. Поэтому в длинных документах, долгих переписках или больших базах знаний нужны дополнительные инженерные приёмы, иначе модель забудет начало диалога.
