...
description: "Как на самом деле работает электронная почта: что происходит с письмом от нажатия «Отправить» до появления во «Входящих», что такое SMTP и почтовые серверы, почему письма попадают в спам и как устроена защита от подделки отправителя. Экспертный разбор."
date: 2026-07-01
category: expert
canonical: https://chimitdorzhi.tech/blog/kak-rabotaet-elektronnaya-pochta-2027/
---
...
**Коротко:** Письмо проходит через сервер отправителя, DNS-запрос к MX-записи домена получателя и сервер получателя, прежде чем попасть во «Входящие». За отправку отвечает протокол SMTP, за получение — IMAP или POP3. В спам письма попадают из-за плохой репутации отправителя, подозрительного содержания или отсутствия SPF, DKIM и DMARC записей в DNS домена.
...
Есть и обратная сторона: правильно настроенный домен с хорошей репутацией и корректными записями аутентификации доставляет письма стабильно и попадает в спам заметно реже. Это напрямую влияет на выручку, если компания рассылает счета, коммерческие предложения или работает с email-маркетингом. Если вы подозреваете, что часть писем от компании не доходит до клиентов, или планируете подключить корпоративную почту, рассылки или интеграцию уведомлений на сайте, имеет смысл [обсудить проект](https://chimitdorzhi.tech/predlozheniya/) и проверить настройки домена вместе со специалистом — это разовая работа, которая закрывает проблему на годы вперёд.

## Частые вопросы

**Почему письма от моей компании попадают в спам?** Чаще всего из-за отсутствия или неправильной настройки SPF, DKIM и DMARC записей домена, из-за низкой репутации отправителя или из-за содержания письма, которое фильтры расценивают как рекламное или подозрительное.

**Как объяснить простыми словами, что такое SPF и DKIM?** SPF — это список серверов, которым разрешено отправлять почту от имени вашего домена. DKIM — это цифровая подпись письма, которая доказывает, что оно не подделано и действительно отправлено владельцем домена.

**Можно ли вообще подделать адрес отправителя письма?** Технически да, поле «От кого» можно подставить произвольным значением. Именно поэтому существуют SPF, DKIM и DMARC — без этих записей получатель не может достоверно проверить, что письмо действительно от вас, и такую подделку легко принять за настоящее письмо.

**Нужен ли мне свой почтовый сервер для малого бизнеса?** Разворачивать собственный сервер обычно не требуется — для большинства компаний достаточно готового облачного решения вроде Яндекс 360 или Google Workspace, подключённого к своему домену. Важнее не сам сервер, а правильная настройка DNS-записей для этого домена.

**Как проверить, что почта моего домена настроена правильно?** Есть бесплатные онлайн-инструменты, которые проверяют MX, SPF, DKIM и DMARC записи домена и показывают, что настроено верно, а что отсутствует. Если вы не уверены в результатах — лучше показать их специалисту, чтобы не пропустить критичную ошибку.
...
**Оригинал статьи:** https://chimitdorzhi.tech/blog/kak-rabotaet-elektronnaya-pochta-2027/
