...
## Альтернатива: GraphHopper и почему решает лицензия

У ORS есть прямой конкурент на тех же данных OpenStreetMap — GraphHopper. Оба написаны на Java, оба разворачиваются как отдельный веб-сервер, оба считают маршруты, время в пути и пошаговые инструкции. Более того, сам ORS вырос из GraphHopper и до сих пор ссылается на него в своей документации. Выбирать между ними по списку возможностей почти бесполезно — он совпадает по большей части.

Выбирать надо по двум вещам, и первая из них — лицензия.

**ORS распространяется под GPL-3.0, GraphHopper — под Apache 2.0.** Для компании, которая просто поднимает сервис у себя и пользуется им внутри, разницы почти нет. Но если вы встраиваете движок маршрутов в собственный продукт и продаёте этот продукт клиентам, разница становится решающей: GPL накладывает обязательства по раскрытию кода производной работы, Apache 2.0 — нет. Это тот случай, когда выбор технологии определяется не характеристиками, а тем, что вы собираетесь делать с результатом.

Вторая вещь — общественный транспорт. GraphHopper умеет работать с данными GTFS, то есть строить маршруты с пересадками на автобусах и электричках, а не только для машины, велосипеда и пешехода. Плюс поддерживает данные о высотах рельефа, что заметно меняет расчёт для велосипедных и пеших маршрутов в холмистой местности. Если в задаче есть расписания транспорта или перепады высот, выбор сужается до одного варианта.

По зрелости оба проекта в порядке: GraphHopper существует с 2012 года, у него больше семи тысяч коммитов и правки приходят ежедневно. Так что вопрос здесь не в надёжности, а в лицензии и в том, нужен ли вам транспорт.
