...
**Коротко:** Качество ответов ИИ-бота проверяют методом LLM-as-Judge: отдельная нейросеть оценивает диалоги бота по чек-листу конкретных вопросов с ответом да или нет — факты верные, ответ по делу, тон уместный, нет выдумок. Это точнее оценки от 1 до 10, показывает, какой критерий проседает, но требует периодической ручной сверки, так как судья тоже может ошибаться.
