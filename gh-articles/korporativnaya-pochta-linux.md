...
**Коротко:** Собственный почтовый сервер на связке Postfix, Dovecot и Rspamd избавляет бизнес от подписок облачных провайдеров и рисков блокировки, но требует правильных DNS-записей (A, MX, PTR), настройки SPF, DKIM и DMARC и защиты трафика через TLS и Let's Encrypt. Защита от спама строится на Rspamd с байесовскими фильтрами и серыми списками, а решение окупается за несколько месяцев при квалифицированном внедрении.
...
Отдельно на эту тему: [Ускорение 1С: тонкая настройка СУБД PostgreSQL на Linux без по](/blog/uskorenie-1c-postgresql/).
