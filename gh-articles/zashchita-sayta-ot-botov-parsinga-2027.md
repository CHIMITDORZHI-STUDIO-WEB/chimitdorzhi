...
description: "Защита сайта от парсинга, ботов и накрутки без вреда SEO: как парсят цены и контент, что работает (WAF, rate limit, ловушки) и что вредит. Пошагово."
date: 2026-06-06
category: security
...
**Коротко:** защита строится многослойно: rate limiting ограничивает частоту запросов, поведенческий анализ и фингерпринт устройства отличают ботов от людей, WAF фильтрует явные атаки, а капча ставится точечно только в местах риска — форма заявки, регистрация, оформление заказа. YandexBot и Googlebot всегда пропускаются без ограничений, их проверяют по обратному DNS, а не по User-Agent.
...
Отдельно скажу про новый класс угрозы, который меняет расстановку сил. Раньше парсер выдавал себя техническими признаками: подозрительный user-agent, отсутствие типичных заголовков, слишком ровный ритм запросов. Сейчас появились инструменты, которые подключают ИИ-агента к настоящему браузеру Chrome и водят его почти как человек, включая перехват трафика и работу через отладочный протокол браузера. С точки зрения фингерпринта это обычный настоящий Chrome, и ловить его по «неправильному» отпечатку становится бесполезно.

Практический вывод, который я делаю для клиентов: центр тяжести защиты смещается с распознавания «робот или человек» на поведение и экономику доступа. Работают вещи, которые не зависят от того, насколько правдоподобен браузер: лимиты на объём выданных данных в единицу времени, а не только на число запросов; отслеживание аномалий по бизнес-логике (один аккаунт просмотрел весь каталог за час, чего живой человек не делает); обязательная авторизация для доступа к чувствительным срезам вроде оптовых цен; и водяные знаки в данных, по которым потом видно, откуда утёк прайс. Полностью закрыться от такого доступа нельзя, и обещать это я не стану — но можно сделать сбор данных дороже, медленнее и заметнее, а это и есть рабочая цель защиты.
