...
**Коротко:** чтобы установить Ollama на Windows, скачайте установщик с ollama.com, запустите его и выполните в терминале команду ollama run llama3 — модель скачается и сразу откроет чат. Для комфортной работы нужно от 8 ГБ оперативной памяти, а видеокарта NVIDIA ускоряет ответы в разы. Данные при этом остаются на вашем компьютере и никуда не передаются.
...
**Где хранятся скачанные модели и можно ли изменить папку?** По умолчанию модели скачиваются в профиль пользователя на системном диске. Если места мало, можно указать другую папку через переменную окружения OLLAMA_MODELS — это удобно, если планируете держать сразу несколько крупных моделей.
