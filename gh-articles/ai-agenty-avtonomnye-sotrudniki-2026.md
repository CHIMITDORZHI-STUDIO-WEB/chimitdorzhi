...
**Коротко:** Multi-agent система — это команда специализированных AI-агентов (researcher, writer, reviewer, executor) под управлением оркестратора, которые вместе выполняют функцию целого отдела вместо одного человека. Архитектуры: supervisor, sequential, hierarchical, swarm. Такая система дороже одиночного агента в 3-8 раз по токенам и нужна только там, где один агент деградирует на сложности задачи.
