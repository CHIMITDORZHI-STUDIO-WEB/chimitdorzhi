...
**Коротко:** В кафе и салоне красоты бумажная штамп-карта плохо работает, потому что теряется и никак не напоминает о себе между визитами. Цифровые баллы, привязанные к номеру телефона или мессенджеру, решают эту проблему: они не теряются, сами присылают напоминание вернуться и позволяют выстроить механику из баллов за визит, бонуса на день рождения и реферального вознаграждения за друга.
...
**По каким показателям понять, что программа лояльности реально работает?** Смотрите на три метрики: долю повторных визитов от общего числа клиентов, средний интервал между визитами одного клиента и долю выручки, которая приходится на держателей карты. Рост доли повторных визитов на 10-15% уже говорит о том, что механика работает.

**Нужно ли менять кассовое оборудование, чтобы запустить баллы?** Нет, обычно цифровая программа лояльности подключается отдельным слоем поверх существующей кассы: администратор вносит сумму чека в приложение или бот, а баллы начисляются автоматически, без переделки самой кассы.
